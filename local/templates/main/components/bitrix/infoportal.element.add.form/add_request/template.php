	...
	<table class="data-table">
		...
		<tbody>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
				<tr <?if($propertyID=="NAME" || $propertyID=="DATE_ACTIVE_TO"):?>style="display:none"<?endif;?>>
							...
							case "N":
								for ($i = 0; $i<$inputNum; $i++)
								{
                                    ...
                                    if($propertyID=="DATE_ACTIVE_TO"){
                                        $value = date("d.m.Y", strtotime("+".$arParams["DATE_VALIDITY"]." month"));
                                    }
								?>
								<input type="text" class="x3-request-form__input--<?=$propertyID?>" <?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>required<?endif?> name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /><?
