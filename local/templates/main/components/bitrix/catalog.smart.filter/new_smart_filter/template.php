...
<div class="bx-filter">
	<div class="bx-filter-section">
		<form name="<? echo $arResult["FILTER_NAME"] . "_form" ?>" action="<? echo $arResult["FORM_ACTION"] ?>" method="get" class="smartfilter">
			<!-- mobile variant of filter -->
			<div class="mmenu menu__filter hide show-m">
				<a href="#mmenu" class="mmenu__opener">
					<span class="filter-opener__title">Фильтры</span>
					<div class="filter-opener__icon">
						<svg aria-hidden="true" width="100%" height="100%">
							<use xlink:href="#filter-opener-icon"></use>
						</svg>
					</div>
				</a>
			</div>
			<nav id="mmenu">
				<button title="Закрыть" type="button" class="btn-close">×</button>
				<ul class="hide">
					<li><a href="/"></a></li>
				</ul>
				<div class="mm-filter__wrapp js-mm-filter" data-tabs-container>
					<?
					// mmenu cycle (not prices) 
					...
					?>
						...
						<div class="mm-filter filter_wrapper">
							<div data-tab-anchor="<?= $arItem["ID"] ?>" class="mm-filter__toggle">
								<span class="filter-title-inner">
									<span class="filter-title-name">
										<?= $arItem["NAME"] ?>
									</span><!--<i class="filter-curr-value">500-1300</i>-->
									<span class="filter-count-selected"></span>
									<span class="filter-title_info"></span>
								</span>
							...
							<div data-tab-target="<?= $arItem["ID"] ?>" class="mm-filter__target">
								<div class="curr_filter_values">
									<?
											...
											<div class="col-xs-12">
												<div class="radio">
													<label class="bx-filter-param-label" for="<? echo "all_" . $arCur["CONTROL_ID"] ?>">
														<span class="row filter-checbox-item">
												...
												<? foreach ($arItem["VALUES"] as $val => $ar) : ?>
													<div class="radio">
														<label data-role="label_<?= $ar["CONTROL_ID"] ?>" class="bx-filter-param-label" for="<? echo $ar["CONTROL_ID"] ?>">
															<span class="row filter-checbox-item <? echo $ar["DISABLED"] ? 'disabled' : '' ?>">
											...
											<? foreach ($arItem["VALUES"] as $val => $ar) : ?>
												<div class="checkbox">
													...
													<span class="row filter-checbox-item">
														...
														<label data-role="label_<?= $ar["CONTROL_ID"] ?>" class="bx-filter-param-label fake-click <? echo $ar["CHECKED"] ? 'is-active-fake' : '' ?> <? echo $ar["DISABLED"] ? 'disabled' : '' ?>" for="<? echo $ar["CONTROL_ID"] ?>"><?= $ar["VALUE"]; ?> </label>
													</span>
												</div>
											<? endforeach; ?>
									<?
				...
				<div class="mm-filter__btns">
					<input class="btn-main filter-apply submit-filter" type="submit" id="set_filter" name="set_filter" value="<?= GetMessage("CT_BCSF_SET_FILTER") ?>" />

					<input class="btn-main filter-cancel exit-filter" value="<?= GetMessage("CT_BCSF_DEL_FILTER") ?>" />
				</div>
			</nav>

			<!-- end mobile variant of filter -->

			<? foreach ($arResult["HIDDEN"] as $arItem) : ?>
				<input type="hidden" name="<? echo $arItem["CONTROL_NAME"] ?>" id="<? echo $arItem["CONTROL_ID"] ?>" value="<? echo $arItem["HTML_VALUE"] ?>" />
			<? endforeach; ?>
			<div class="layout__filter hide-m">
				...
				foreach ($arResult["ITEMS"] as $key => $arItem) {
					...
					<div class="filter-wrap filter_wrapper">
						<div class="filter-title">
							<div class="filter-toggle bx-filter-parameters-box <? if ($arItem["DISPLAY_EXPANDED"] == "Y") : ?>bx-active<? endif ?>" onclick="smartFilter.hideFilterProps(this)">
								<span class="bx-filter-container-modef"></span>
								<span class="filter-title-inner">
									<span class="filter-title-name">
										<?= $arItem["NAME"] ?>
								...
								<div class="exit-filter">
									<svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
										<path d="M1 9L9 1" stroke="#E08F40" stroke-width="2" stroke-linecap="round" />
										<path d="M1 1L9 9" stroke="#E08F40" stroke-width="2" stroke-linecap="round" />
									</svg>
								</div>
							</div>
							...
							<div class="filter-popup">
								<div class="bx-filter-block" data-role="bx_filter_block">
									<div class="row bx-filter-parameters-box-container">
										<div class="curr_filter_values">
											...
											?>
													<div class="range-slider-input row is-grid flex-center">
														...
														<div class="cell-12 bx-ui-slider-track-container">
															<div class="bx-ui-slider-track" id="drag_track_<?= $key ?>">
																<?
																$precision = $arItem["DECIMALS"] ? $arItem["DECIMALS"] : 0;
														...
														<div class="cell-6 bx-filter-parameters-box-container-block bx-left">
															<i class="bx-ft-sub"><?= GetMessage("CT_BCSF_FILTER_FROM") ?></i>
															<div class="bx-filter-input-container">
																<input class="min-price" type="text" name="<? echo $arItem["VALUES"]["MIN"]["CONTROL_NAME"] ?>" id="<? echo $arItem["VALUES"]["MIN"]["CONTROL_ID"] ?>" value="<? echo $arItem["VALUES"]["MIN"]["HTML_VALUE"] ?>" placeholder="<? echo $arItem["VALUES"]["MIN"]["VALUE"] ?>" size="5" onkeyup="smartFilter.keyup(this)" />
															</div>
														</div>
														<div class="cell-6 bx-filter-parameters-box-container-block bx-right">
															<i class="bx-ft-sub"><?= GetMessage("CT_BCSF_FILTER_TO") ?></i>
															<div class="bx-filter-input-container">
																<input class="max-price" type="text" name="<? echo $arItem["VALUES"]["MAX"]["CONTROL_NAME"] ?>" id="<? echo $arItem["VALUES"]["MAX"]["CONTROL_ID"] ?>" value="<? echo $arItem["VALUES"]["MAX"]["HTML_VALUE"] ?>" placeholder="<? echo $arItem["VALUES"]["MAX"]["VALUE"] ?>" size="5" onkeyup="smartFilter.keyup(this)" />
															</div>
														</div>
														<?
														$arJsParams = array(
										...
										<input class="btn-main filter-apply" type="submit" id="set_filter" name="set_filter" value="<?= GetMessage("CT_BCSF_SET_FILTER") ?>" />
									</div>
								</div>
							</div>
						</div>
