...
\Site\Main\Css::showInlineCssBlock(array(SITE_TEMPLATE_PATH . "/css/guestcard.css"));
?>

<div class="x3-request">
<?if(!empty($_REQUEST["hash"])):?>
    <div style="text-align: center">
        <?if(!empty($arResult["MESSAGE"])):?>
            <div>
                <?=$arResult["MESSAGE"]?><br><br><br>
            </div>
        <?endif?>

        <img src="<?=$arResult["QUESTCARD_IMG"]?>" alt=""><br><br><br>
        <a class="x3-btn" href="<?=$arResult["QUESTCARD_IMG"]?>" download>Скачать</a>
    </div>
<?$APPLICATION->SetTitle("Поздравляем! Ваша карта готова.");?>
<?else:?>
    ...
    <form id="x3-request-form" name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
        ...
        <?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
            ...
            <?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
                <div class="x3-request-form__item" <?if($propertyID=="NAME" || $propertyID=="DATE_ACTIVE_TO"):?>style="display:none"<?endif;?>>
                    <?if($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "S"):?>
                        <div class="x3-request-form__title">
                        ...
                        switch ($INPUT_TYPE):
                            ...
                            case "N":
                                for ($i = 0; $i<$inputNum; $i++)
                                {
                                    ...
                                    if($propertyID=="DATE_ACTIVE_TO"){
                                        $value = date("d.m.Y", strtotime("+".$arParams["DATE_VALIDITY"]." day"));
                                    }
                                    ?>
					<input placeholder="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["NAME"]?><?/*if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>*<?endif;*/?>" type="text" class="x3-request-form__input--<?=$propertyID?>" <?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"])):?>required<?endif?> name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="25" value="<?=$value?>" /><?
                            ...
                            case "L":
                                ...
                                switch ($type):
                                    ...
                                    case "radio":
                                        ...
                                        <div class="x3-request-form__check">
            ...
            <?if($arParams["USE_CAPTCHA"] == "Y" && $arParams["ID"] <= 0):?>
				<br>
                <div class="x3-request-form__item x3-captcha">
                    <div class="x3-captcha__img">
                        <div>
                            <input type="hidden" name="captcha_sid" value="<?=$arResult["CAPTCHA_CODE"]?>" />
                            <img src="/bitrix/tools/captcha.php?captcha_sid=<?=$arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" />
                        </div>
                    </div>
                    <div class="x3-captcha__input">
        ...
        <div class="x3-request__agrement">
			<?/*
            <div>
                <input type="checkbox" id="agrement-check">
                <label for="agrement-check">Даю согласие на <a href="/politika/">обработку своих персональных данных</a>*</label>
            </div>
			*/?>
			<?/*<span>* Все поля, отмеченные звездочкой являются обязательными для заполнения.</span>*/?>
			<?$APPLICATION->IncludeComponent(
				"bitrix:main.include",
				"",
				Array(
					...
					"EDIT_TEMPLATE" => "",
					"PATH" => "/local/templates/main/include/guestcard.php"
				)
			);?>
        </div>

        <div class="x3-request-form__bottom">
                <input class="x3-btn" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
