		...
		if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
		{
			...
			if (count($arResult["ERRORS"]) == 0)
			{
				...
				else
				{
					...
					$sAction = "ADD";
                    $arResult["ELEMENT_ID"] = $oElement->Add($arUpdateValues, $bWorkflowIncluded, true, $arParams["RESIZE_IMAGES"]);
					if (!$arParams["ID"] = $arResult["ELEMENT_ID"])
					{
						$arResult["ERRORS"][] = $oElement->LAST_ERROR;
					}
					else
					{
                        $arResult["HASH"] = md5($arResult["ELEMENT_ID"].$arUpdateValues["NAME"]);
			...
			if (count($arResult["ERRORS"]) == 0)
			{
                ...
                $sRedirectUrl .= "&hash=".$arResult["HASH"];
