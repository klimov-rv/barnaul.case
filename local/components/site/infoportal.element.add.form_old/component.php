...
//                        $QrCodeUrl = "https://guestcard.mihail.bxtest.ru/?request-id=".$arParams["ID"];
//                        $QrCodeImg = CFile::MakeFileArray(Site\Main\Guestcard::createQrCode($QrCodeUrl, md5($arParams["ID"].$arUpdateValues["NAME"])));
//                        $arUpdateValues["PREVIEW_PICTURE"] = $QrCodeImg;
						...
						if($arParams["SEND_EMAIL"] == "Y")
						{
							...
							else 
							{
								...
								"EMAIL_TO"    => $arUpdateValues["PROPERTY_VALUES"][4],
								"IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
								"IBLOCK_ID" => $arParams["IBLOCK_ID"],
								"ID" => $arParams["ID"],
								"SECTION_ID" => $SECTION_ID,
                                "DATE_ACTIVE_FROM" => FormatDate("d.m.Y", strtotime($arUpdateValues["DATE_ACTIVE_FROM"])),
                                "DATE_ACTIVE_TO" => $arUpdateValues["DATE_ACTIVE_TO"],
                         	);

                            CEvent::Send($arParams["EVENT_NAME"], SITE_ID, $arFields, "N", "", array($arUpdateValues["PREVIEW_PICTURE"]["tmp_name"]));
                            */
						}
					}
		...
		if ($bShowForm)
		{
			...
			$arResult["ишекш"] = htmlspecialcharsex($_REQUEST["strIMessage"]);
